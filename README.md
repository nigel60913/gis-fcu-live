...
- 主持控制台點選「開啟抽獎頁」後，會另開 `lottery.html`。
- 抽獎頁會彙整所有題目的回答資料，優先以既有 `clientId` 去重（缺少 ID 的舊資料才以標準化暱稱去重）後隨機抽出最多 20 位。
- 抽獎動畫與音樂共 5 秒，公布後可匯出 UTF-8 CSV 得獎名單。
- 抽獎頁僅呈現福利點數核心概念，不放置完整 PDF。
